...
int main(int /*argc*/, char* /*argv[]*/) 
{
	...
	//TClass::GetClass("EdwHLAMuonModuleSubRecord")->IgnoreTObjectStreamer();
	/*
	TFile f("new.root","recreate");
	cout << "Open Root File" << endl;
	
	EdwHLAEvent *mEv = EdwEventFactory::NewHLAEvent();
	//EdwHLAEvent *mEv = new EdwHLAEvent;
	cout << "Created EdwHLAEvent" << endl;
	
	TTree *t = new TTree("t","EdwEventTree");
	cout << "Created TTree" << endl;
	
	t->Branch("event","EdwHLAEvent",&mEv,32000,99); 
	...
	//t->GetBranch("fEdwHLABoloSysRecord")->SetName("boloSys");
	//t->GetBranch("fEdwHLAMuonVetoSysRecord")->SetName("muonSys");
...
*/
	...
	EdwDSWriter mOutput("new.root","HLA","recreate");
	EdwHLAEvent *mEv = dynamic_cast<EdwHLAEvent*>(mOutput.GetEvent());
		
	/*if(mEv == 0){
		cout << "EdwDSWriter returned a NULL pointer from GetEvent" << endl;
		return -1;
	}*/
