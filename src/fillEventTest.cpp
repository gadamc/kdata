...
int main(int argc, char* argv[]) 
{
	...
	TTree *t = new TTree("KT","KEventTree");
	...
	for(Int_t i = 0; i < numEvents; i++){
		...
		if(gRandom->Rndm() < 0.05){
			...
			for (Int_t i = 0; i < nBolo; i++) {
				bIsBoloEvent = true;
				KHLASingleBoloSubRecord* mBolo2 = mEv->AddSingleBolo();
	...
	cout << "Open Root File" << endl;
	KTestEventClass *mTEv = new KTestEventClass();
	cout << "Created KTestEventClass" << endl;
	TTree *tt = new TTree("KTest","KEventTreeTest");
	cout << "Created TTree" << endl;
	tt->Branch("KTestEventBranch","KTestEventClass",&mTEv,32000,99); 
	...
	cout << "Closing File" << endl;
	delete mTEv;
	cout << "Clean up KTestEvent object " << endl;
	 */
}
