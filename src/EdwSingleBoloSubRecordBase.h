...
#ifndef __EDWSINGLEBOLOSUBRECORDBASE_H__
#define __EDWSINGLEBOLOSUBRECORDBASE_H__
...
class EdwSingleBoloSubRecordBase : public EdwSubRecordBase {

public:
  //Constructors
  EdwSingleBoloSubRecordBase(void);
	EdwSingleBoloSubRecordBase(const EdwSingleBoloSubRecordBase &aRec);
	EdwSingleBoloSubRecordBase& operator=(const EdwSingleBoloSubRecordBase &aRec);
  virtual ~EdwSingleBoloSubRecordBase(void);
	virtual void Clear(Option_t *opt = "C");
	Bool_t IsSame(const EdwSingleBoloSubRecordBase &aRec, Bool_t bPrint = false) const;
	Bool_t operator==(const EdwSingleBoloSubRecordBase &aRec) const { return IsSame(aRec,false); }
	Bool_t operator!=(const EdwSingleBoloSubRecordBase &aRec) const { return !(*this==aRec); }
...
private:
  ...
  void InitializeMembers(void);
	void CopyLocalMembers(const EdwSingleBoloSubRecordBase &aRec);
	
  ClassDef(EdwSingleBoloSubRecordBase,1);
};


#endif // __EDWSINGLEBOLOSUBRECORDBASE_H__
