...
int main(int argc, char* argv[])
{
	...
	EdwDSReader fin(fInFileName.Data());
	EdwHLAEvent *fEventIn = dynamic_cast<EdwHLAEvent *>(fin.GetEvent());
	if(fEventIn == 0){
	...
	for(Int_t i = 0; i < fin.GetEntries(); i++){
		...
		if(i % fBreakAfterThisNumEvents == 0){
			...
			fout = new EdwDSWriter(fOutName.Data());
			fEventOut = dynamic_cast<EdwHLAEvent *> (fout->GetEvent());
			if(fEventOut == 0){
				cout << "Hmmmm EdwDSWriter gave me the wrong event type" << endl;
				return -1;
			}
