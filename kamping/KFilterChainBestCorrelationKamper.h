...
public:
  ...
  std::map<std::string, KResult>  MakeKamp(KRawBoloPulseRecord * , double ){map<string, KResult> myResults; return myResults;}
  //std::map<std::string, KResult>  MakeKamp(KRawBoloPulseRecord * rawPulseRecord){return MakeKamp(rawPulseRecord, -1.0);}
  ...
  template <class T> double TestFunc(const std::vector<T> &aPulse, unsigned int i, const char* name);

  //std::map<std::string, KResult>  MakeKamp(const std::vector<double> &aPulse, double fixPeakPosition = -1.0, const char* channelName = ""){fPreProcessor->SetInputPulse(aPulse); return MakeKampHack(fixPeakPosition,channelName);}
  //std::map<std::string, KResult>  MakeKamp(const std::vector<short > &aPulse, double fixPeakPosition = -1.0, const char* channelName = ""){fPreProcessor->SetInputPulse(aPulse); return MakeKampHack(fixPeakPosition,channelName);}
  //std::map<std::string, KResult>  MakeKamp(const std::vector<float > &aPulse, double fixPeakPosition = -1.0, const char* channelName = ""){fPreProcessor->SetInputPulse(aPulse); return MakeKampHack(fixPeakPosition,channelName);}
  //std::map<std::string, KResult>  MakeKamp(const std::vector<int   > &aPulse, double fixPeakPosition = -1.0, const char* channelName = ""){fPreProcessor->SetInputPulse(aPulse); return MakeKampHack(fixPeakPosition,channelName);}
...
//  std::map<std::string, KResult> MakeKampHack(double fixPeakPosition, const char* channelName);
  

};

template <class T> double  KFilterChainBestCorrelationKamper::TestFunc(const std::vector<T> &aPulse, unsigned int i, const char* name)
{
  std::cout << name << " "  << aPulse.at(i) << std::endl;
  double myResults;
  myResults = aPulse.at(i);
  return myResults;
}
