...
def kdatafileregex():
  '''
    ...
    returns a regular expression string that matches a kdata file (aka 'partition') string.
    something like 'ma22a012_023.root', for example. may include .amp.root and .hla.root files.
    you can use this in the python re module.
  
  '''
  return sambadatafileregex().rstrip('$') + '(\.root|\.amp\.root|\.hla\.root)$'

def kdatarawfileregex():
  '''
  
    returns a regular expression string that matches a kdata raw-level file (aka 'partition') string name.
    something like 'ma22a012_023.root', for example.
    you can use this in the python re module.
  
  '''
  return sambadatafileregex().rstrip('$') + '\.root$'
...
def isvalidkdatarawfilename(name):
  '''
  
    Test if name is a valid name for KData raw-level file (aka partition) names. Such as ma22000_012.root

  '''
  if re.match( kdatarawfileregex() , name) is None: 
    return False
  else: 
    return True
