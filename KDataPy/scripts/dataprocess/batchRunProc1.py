...
def submitProc1BatchJobs(scriptOut, scriptErr, script, server, dbname, docids):
  ...
  command = 'qsub -P P_edelweis -o %s -e %s -l sps=1 -l ct=100000 -l vmem=4G -l fsize=512M  %s %s %s useProc0=True %s' % (scriptOut, scriptErr, script, server, dbname, ' '.join(docids) ) 
