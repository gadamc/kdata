...
int concatKDS(string *inputFiles, Int_t numFiles, string outputFile){
  ...
  for(Int_t i=0; i< numFiles; i++){
    ...
    mFilesToMerge[i] = TFile::Open(inputFiles[i].c_str());

    if(mFilesToMerge[i]->Get("t") == 0){
      cout << " woops! File doesn't seem to be an KData File. Quitting." << endl;
      return  -1;
    }
    mList->Add((TTree *)mFilesToMerge[i]->Get("t"));
  }
  ...
  TTree *outTree = (TTree *)TTree::MergeTrees(mList, "fast");
