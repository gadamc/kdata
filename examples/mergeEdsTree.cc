...
const Char_t kEDSTreeTitle[] = "EdwDSTree"; 
const Char_t kEDSBranchClass[] = "EdwHLAEvent";
...
EdwHLAEvent* mergeBoloSysRecord(EdwHLAEvent* ev1, EdwHLAEvent* ev2){
	string n1, n2;
	string h1, h2;
...
EdwHLAEvent* mergeMuonVetoSysRecord(EdwHLAEvent* ev1, EdwHLAEvent* ev2){
...
EdwHLAEvent* mergeSubRecords(EdwHLAEvent* ev1, EdwHLAEvent* ev2){ 
	...
	for(Int_t i =0; i<bolo2;i++){
		EdwHLASingleBoloSubRecord* singleBoloSub=ev1->AddBolo();
		EdwHLASingleBoloSubRecord* singleBoloSub2=(EdwHLASingleBoloSubRecord*)boloArray2->At(i);
		*singleBoloSub = *singleBoloSub2; 
		EdwHLASambaSubRecord* sambaSub2 = singleBoloSub2->GetSambaRecord();
		EdwHLASambaSubRecord* sambaSub = 0;
	...
	for(Int_t i =0; i<muon2;i++){
		EdwHLAMuonModuleSubRecord* muonModuleSub=ev1->AddMuonModule();
		EdwHLAMuonModuleSubRecord* muonModuleSub2=(EdwHLAMuonModuleSubRecord*)muonArray2->At(i);
...
EdwHLAEvent* mergeEdsEvent(EdwHLAEvent* ev1, EdwHLAEvent* ev2){
...
void printInfo(EdwHLAEvent* ev){
	ev->myPrint();
	ev->myPrintB();
	if((ev->GetNumSambas())!=0){
		TClonesArray* sambaArray=ev->GetSambaSubRecords();
		EdwHLASambaSubRecord* sam=(EdwHLASambaSubRecord*)sambaArray->At(0);
...
bool checkSpecialCase(string in, EdwHLAEvent* ev ){
...
int mergeEdsTree(string inputPath1, string inputPath2, string outputPath, string log){
	...
	EdwDSReader inFile1(inputPath1.c_str());
	EdwHLAEvent *oldEv1 = dynamic_cast<EdwHLAEvent *>(inFile1.GetEvent());
	if(oldEv1 == 0) return -1;
	
	EdwDSReader inFile2(inputPath2.c_str());
	EdwHLAEvent *oldEv2 = dynamic_cast<EdwHLAEvent *>(inFile2.GetEvent());
	if(oldEv2 == 0) return -1;
		
	EdwDSWriter f(outputPath.c_str());
	EdwHLAEvent *mEv = dynamic_cast<EdwHLAEvent *>(f.GetEvent());
	if(mEv == 0) return -1;
...
Bool_t checkTBit(EdwHLAEvent* ev){
	EdwHLAMuonVetoSysRecord *mMvSysRec = ev->GetMuonVetoSystemRecord();	
...
Int_t mergeEdsTree(string inputPath1, string inputPath2, string outputPath, string log, Int_t kEntryOfBoloStart, Bool_t bigNeutron, Int_t debugAt){
	...
	EdwDSReader inFile2(inputPath2.c_str()); //calls GetEntry and leads to a segFault if one opens the bolometer file first, thus keep this sequence
	//cout << "File 2.1"<< endl;
	EdwHLAEvent *oldEv2 = dynamic_cast<EdwHLAEvent *>(inFile2.GetEvent());
	if(oldEv2 == 0) return -1;
	//cout << "File 2.2"<< endl;		
