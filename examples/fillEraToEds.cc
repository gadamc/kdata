...
int eraToEds(string inputPath, string kDetectorName, string outputFile, string log, 
						 string tstNewPath="/kalinka/home/edelweiss/Bolo/Run12/tstnew/")
{
	...
	EdwDSWriter fOutFile(outputFile.c_str());
	...
	for(int i=0; i<entries; i++){// fill entry specific information
		...
		if(sRunName.compare(0,4,"jc27") != 0){  
			...
			EdwHLASingleBoloSubRecord* bolo = mEv->AddBolo();
