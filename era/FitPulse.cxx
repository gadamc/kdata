...
{
  LoadPulse(aPulse,aBasicPreprocess);
}

void FitPulse::LoadPulse(const EdwPulse* aPulse, Bool_t aBasicPreprocess)
{
...
void FitPulse::FindPeaks(Bool_t aCriterium, Bool_t AnySign, Int_t aLength) {
...
vector<Float_t> FitPulse::GetWienerAmpl(const FloatVect& aTemplateFFT, const FloatVect& aNoise, Bool_t aDoFFT, Float_t aOffset, Bool_t aChi2Switch, string aPlotFilter, string aPlotFit) {
  ...
  // Time translation in Fourier space for the template
  // (should not be used excepted if pre- and post-pulses are long enough in the trace)
  if (aOffset != 0 ) {
    UInt_t n = aTemplateFFT.size();
    for (UInt_t i=0; i<= (UInt_t)(n/2); i++) {
      Float_t lAngle = 2*TMath::Pi()*i*aOffset/n ;
      lTemplate[i] = aTemplateFFT[i]*cos(lAngle);
      if (i > 0 && 2*i != n)
				lTemplate[n-i] = aTemplateFFT[n-i]*sin(lAngle); // eg. (i=1 -> n) and (i=n/2-1 -> n/2+2)
    }
  }
  if (aDoFFT) {
    if (!fBasicPreprocessed) this->BasicPreprocess();
  ...
  lDenum.Divide(aNoise);
  FloatVect lNumer = lTemplate;
  lNumer.Multiply(fProcessedTraceFFT);
  lNumer.Divide(aNoise);
  Float_t DenumInt = 1/lDenum.Integral();
...
vector<Float_t> FitPulse::GetFitWiener(const OffsetFFT* aTemplateFFTs, const FloatVect& aNoise, Bool_t aFast, 
																			 Bool_t aSubBin, Float_t aOffsetMin, Float_t aOffsetMax, Float_t aOffsetStep, 
																			 string aPlotFit) {
  ...
  // Plot fit
  if (aPlotFit != "") GetWienerAmpl(lTmpltFFT,aNoise,0,0,0,"",aPlotFit);
