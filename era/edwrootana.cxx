...
void Preprocess_Step() {
  ...
  cout << "** Number of time steps: " << 1+(tbounds[1]-tbounds[0])/timestep<< endl;
  for (time_t tcut=tbounds[0]; tcut < tbounds[1]; tcut+=timestep) {
    time_t t_end=tcut+timestep;
    if ( t_end >= tbounds[1] ) t_end = tbounds[1]+1; // avoid overlap between different runs! 
    if (gPlotDir == "None")
      AnaDb->BasicFillPreprocessData(Chain,lChannels,lPulseBins,tcut,t_end);
    else
      AnaDb->BasicFillPreprocessData(Chain,lChannels,lPulseBins,tcut,t_end,gPlotDir+"/spectrum_"+int2str(i));
...
void Process_Step() {
  ...
  if (gHeatIonSynchroFile == "None") {
    ...
    Ntuple->FillDataWithChannelSynchro(Chain,Channels,AnaDb,gIStart,gIStop,RawDb);
  }
...
void Event_Dump() {
  ...
  for (UInt_t i=0; i<lRuns.size(); i++)
    RawDb->DumpEvent(lRuns[i],lNums[i],gRootEventDir,gASCIIOutputFile);
}
...
void FrequencyWeights_Dump() {
  ...
  if (gChannelListFile != "None") {
    ...
    AnaDb->DumpFrequencyWeights(gASCIIOutputFile,Channels,gWienerWindowStart,gWienerWindowStop);
  } else {
    AnaDb->DumpFrequencyWeights(gASCIIOutputFile,gWienerWindowStart,gWienerWindowStop);
  }
}
