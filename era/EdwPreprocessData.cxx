...
void EdwPreprocessData::DumpInfo() const {
  ...
  cout << "Validity from "<<fStartTime<<" to "<<fEndTime<<" - Run: "<<fRun<<endl;
...
void EdwPreprocessData::ComputeNoiseSpectrum(TChain* aChain) {
  ...
  for (Long64_t ievt=lFirst; ievt<lLast; ievt++) {
	...
	if (lRun != fRun) cout << "Warning : change of run in noise spectrum computation!" <<endl;
      }
      FitPulse lPulse(lEdwPulse);
      if (fNbPtsPatterns.size() != 0) { // hoho!
	if (fNbPtsPatterns[0] != 0) lPulse.SetPatternLength(fNbPtsPatterns[0]);
      }
  ...
  if (fNbTracesForNoiseSpectrum < 10) { // Redo it with extra weak criterium
    cerr << "ComputeNoiseSpectrum : New loop with eXtraWEAK FindPeaks cut. " <<
      fNbTracesForNoiseSpectrum << " traces found."<< endl;
    for (Long64_t ievt=lFirst; ievt<lLast; ievt++) {
      aChain->GetEntry((ULong_t)ievt);
      time_t lTime = lEvt->Header()->DateSec();
      lEdwPulse = lEvt->Pulse(fChannel);
	...
	lPulse.SetBaseFromPeakBin(fPulseBin) ;
	lPulse.FindPeaks(FP_XWEAK,0); // Even less strict cut!
	if (lPulse.GetPeakBins().size() == 0) {
	  if (lPulse.IsHeat()) lPulse.RemoveLinearBase();
	  lPulse.ComputeTraceFFT();
	  FloatVect lSpec = lPulse.ProcessedTraceFFT();
  ...
  if (fNbTracesForNoiseSpectrum < 5) { // Ultimate round: take all traces!
    cerr << "ComputeNoiseSpectrum :  New loop, taking all traces.." << endl;
    for (Long64_t ievt=lFirst; ievt<lLast; ievt++) {
      aChain->GetEntry((ULong_t)ievt);
      time_t lTime = lEvt->Header()->DateSec();
      lEdwPulse = lEvt->Pulse(fChannel);
