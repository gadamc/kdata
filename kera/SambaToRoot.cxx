...
Int_t SambaToRoot::FillRootFile(Int_t aBufSize) {
  ...
  while ( fEdwFile.good() ) {
    ...
    while ( lLine.compare(0,kEnteteVoieSize,kEnteteVoie) ) {
      ...
      if (diese_pos != string::npos && eq_pos != string::npos) {
				string lField = lLine.substr(0,eq_pos-1);
				string lValue = lLine.substr(eq_pos+1,diese_pos-1-eq_pos) ;
    ...
    while (lNopulse == 0) {
      ...
      while ( lLine.compare(0,kSeparatorSize,kSeparator) ) { // Pulse header
				...
				if (diese_pos != string::npos && eq_pos != string::npos) {
					string lField = lLine.substr(0,eq_pos-1);
					// Added grace au format evolutif et toujours plus astucieux dans samba...
